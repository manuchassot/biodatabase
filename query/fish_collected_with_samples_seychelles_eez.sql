-- Fish collected with samples and within the Seychelles waters
...
-- Seychelles EEZ: WHERE eez.iso_3digit LIKE 'SYC'
-- Use of geo_data.eez_land_v2_201410 so that the polygon of the EEZ includes the whole area and islands
...
DROP TABLE IF EXISTS fish_collected_with_samples_seychelles_eez;
CREATE TEMP TABLE fish_collected_with_samples_seychelles_eez AS 
...
AND fish.project NOT IN ('BIOM-BET','BIOMCO','BIOMCO1','PATUDO','IOT-stomachs','DCF-IO')
),
...
--AND analysis_group IN ('FA','SI','O','M')
...
-- Export the table
\copy fish_collected_with_samples_seychelles_eez TO '/home/stagiaire/Emotion3/queries/FISH/fish_collected_with_samples_seychelles_eez.csv' WITH DELIMITER ';' CSV HEADER
